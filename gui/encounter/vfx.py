...
class VFX(widgets.RelativeLayout, EncounterViewComponent):
    ...
    def update(self):
        ...
        self.__cached_vfx.append(self.move_crosshair)
        ...
        for effect in self.api.get_visual_effects():
            ...
            if effect.eid is effect.SPRITE:
                ...
                if 'source' in effect.params:
                    sprite_name = effect.params['source']
                ...
                if 'set_size' in effect.params:
                    set_size = effect.params['set_size']

                sprite_name = Assets.get_sprite('ability', sprite_name)
                ...
                with self.canvas:
                    ...
                    if 'stretch' in effect.params:
                        ...
                        new_pos = cc_int(tuple((np.array(point) + (np.array(offset)))))

                    new_size = cc_int(tuple(np.array(scale) * (sprite.size if set_size is None else set_size)))
            ...
            if effect.eid is effect.SFX and self.api.e.auto_tick:
                ...
                Assets.play_sfx(
                    ...
                    volume=Settings.get_setting('volume_sfx'),
